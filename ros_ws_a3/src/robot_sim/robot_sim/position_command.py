...
class PositionCommander(object):
    ...
    def __init__(self, master, num_joints):
        rclpy.init()
        self.node = rclpy.create_node('position_command')

        self.pub_pos = self.node.create_publisher(JointState, "/joint_positions", 1)
        self.num_joints = num_joints
        self.node.create_subscription(JointState, "joint_states", self.callback, 1)
