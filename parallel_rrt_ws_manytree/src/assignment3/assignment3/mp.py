...
class MoveArm(Node):
    ...
    def motion_planning(self, ee_goal: Transform): 
        ...
        #--------------------------------------
        # Parallelization: Launch multiple threads to find a path
        #--------------------------------------
        ...
        # We start timing for the tree expansion now
        raw_path_start = time.time()

        threads = []
        for i in range(n):
            t = threading.Thread(target=self.find_path, args=(q_current, q_goal, max_iterations, q_result))
            t.start()
            threads.append(t)
        ...
        # We got a solution, now we can optionally terminate other threads if needed
        # but they will naturally complete. We can just not join them or ignore them.
        # In a real implementation we might use an event to signal threads to stop.

        #--------------------------------------
        # Reconstruct path from solution_tree
        #--------------------------------------
        # solution_tree is a list of RRTBranch objects ended with the goal node
        # The last element of the solution_tree is the goal branch
        # Go backwards to get the path
        last_node = solution_tree[-1]
        path = []
        current = last_node
        ...
        validation_time = 0.0  # We didn't measure validation time separately in parallel version
    ...
    def find_path(self, q_current, q_goal, max_iterations, q):
        # This function runs in a separate thread.
        # It attempts to find a path using RRT expansions.
        # If found, it puts the solution (the tree) in the queue q.
        ...
        # If no solution is found, do nothing (the main thread might wait until a solution from another thread)
